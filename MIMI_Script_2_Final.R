...
DoublePeakChecker <- function(Interaction_Matrix) {
    ...
    while (matrix.row.no <= matrix.total.rows) {
        ...
        df.name <- 
            read.csv(paste0("Simplified/Testing Broad-Scale Interactions/OutputFiles/", 
                            cc.name, ".csv"))
...
# Also note: Exported RawData files listing the retention time, peak area and UV
...
MIMI2 <- function() {  
    ...
    while (matrix.row.no <= matrix.total.rows) {
        ...
        # Reads in the the corresponding con files from RawData.

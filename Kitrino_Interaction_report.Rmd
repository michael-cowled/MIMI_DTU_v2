...
```{r, echo = FALSE}
filenames <- list.files(path = "Testing Broad-Scale Interactions/OutputFiles/FvKIT", pattern = "F", full.names = TRUE)
...
heatmap_fvf <- ggplot(data = X.list, aes(x=PeakNo_con, y=Ref_Culture)) + 
    geom_tile(aes(fill=PeakRatio)) +
    scale_fill_gradient2(low = "dark blue", high = "dark red", mid = "white", 
                         midpoint = 0, limit = c(min(X.list$PeakRatio), 
                                                 max(X.list$PeakRatio)), 
                         space = "Lab", name="Ratio (%)") +

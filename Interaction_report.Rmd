...
subset.list <- filter(coculture.list, Ref_Culture == "F01") %>%
  filter(Matched_con == "F01CON" | is.na(Matched_con)) %>%
...
```{r, echo = FALSE, warning = FALSE}
filenames <- list.files(path = "Testing Broad-Scale Interactions/OutputFiles/NT_FvA", pattern = "", full.names = TRUE)
my.data <- lapply(filenames, read.csv)
full.list <- rbindlist(my.data, use.names=TRUE, fill=FALSE)

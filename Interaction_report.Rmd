...
```{r, echo = FALSE}
filenames <- list.files(path = "Testing Broad-Scale Interactions/OutputFiles/Tal_FvF", pattern = "", full.names = TRUE)
...
heatmap.fvf.avg <- ggplot(data = ratio.df, aes(x=Int_Culture, y=Ref_Culture)) + 
    geom_tile(aes(fill=Avg_Ratio)) +
    scale_fill_gradient2(low = "dark blue", high = "dark red", mid = "white", 
                         midpoint = 0, space = "Lab", name="\u03b1 (%)", limit = c(-80,80)) +
    labs(x="Interacting Culture", y="Reference Culture") +
    theme_grey(base_size=8) +
    theme(axis.text.x=element_text(angle=45, hjust = 1))
heatmap.fvf.avg
...
subset.list <- filter(coculture.list, Ref_Culture == "FP1927") %>%
  filter(Matched_con == "FP1927CON" | is.na(Matched_con)) %>%
    select(Ref_Culture, Matched_con, Int_Culture, PeakNo_con, PeakRatio) %>%
...
heatmap_fvf <- ggplot(data = subset.list, aes(x=PeakNo_con, y=Int_Culture)) + 
    ...
    scale_x_continuous(breaks=c(0, 5, 10, 15, 20, 25, 30, 35, 40), limits=c(1,40)) +
    scale_fill_gradient2(low = "dark blue", high = "dark red", mid = "white", 
...
```{r, echo = FALSE, warning = FALSE}
mean.list <- setnames(data.frame(matrix(ncol = 16, nrow = 15)), c("mE5", "mE6", "mE5&6", "mAll",
                                                                 "eE5", "eE6", "eE5&6", "eAll",
                                                                 "nE5", "nE6", "ne5&6", "nAll",
                                                                 "pE5", "pE6", "pe5&6", "pAll"))
...
res <- t.test(Effect_5$RetTime_CC, full.list$RetTime_CC, alternative = "greater", var.equal = FALSE)
mean.list[i,13] <- res$p.value
}
if (length(Effect_6$RetTime_CC) > 1) {
res <- t.test(Effect_6$RetTime_CC, full.list$RetTime_CC, alternative = "greater", var.equal = FALSE)
mean.list[i,14] <- res$p.value
}
if (length(Effect_5_6$RetTime_CC) > 1) {
res <- t.test(Effect_5_6$RetTime_CC, full.list$RetTime_CC, alternative = "greater", var.equal = FALSE)
mean.list[i,15] <- res$p.value
}
if (length(full.list$RetTime_CC) > 1) {
res <- t.test(full.list$RetTime_CC, full.list$RetTime_CC, alternative = "greater", var.equal = FALSE)

...
# by MQ Research Training Program (RTP) Scholarship.
...
# 1.ReadExcel - Reads in the RawData files corresponding to a particular row number 
# in the Interaction_Matrix. See MIMI_Code_Book for data formatting.
...
#Reads in RawData .csv file and reformats UV column to be more usable.
...
ReadUV <- function(excel.name) {
    uv.name <- read.csv(paste0("Simplified/Testing Broad-Scale Interactions/RawData/",
                                 excel.name, "_uv.csv"))
    return(uv.name)
}
...
# CON2	
# Coculture
# Note: Exported RawData files listing the retention time, peak area and UV
# maxima for each peak should be present for each control and coculture.
...
MIMI <- function() {           
    ...
    while (matrix.row.no <= matrix.total.rows) {
        ...
        write.csv(refined.cc.df, 
                  paste0("Simplified/Testing Broad-Scale Interactions/OutputFiles/", 
